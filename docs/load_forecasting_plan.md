...
当前数据只有 4 个场站，样本量中等，因此第一版模型不宜过大。建议从中等偏小配置起步，再在验证集上做有限搜索。
...
- `patch_len = 24` 对应 `2` 小时感受野，适合作为第一版的局部模式单元
- 若更关注高频抖动，可优先尝试 `patch_len = 12`
...
由于本方案采用 `RevIN` 对输入输出做窗口级标准化，因此 `Huber Loss` 建议优先在归一化空间计算。

在该前提下，`delta` 使用归一化后的无量纲值更合理，第一版建议：
